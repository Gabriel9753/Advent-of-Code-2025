...
def preprocess_input(input_data):
    return [np.array(list(map(int, x.split(",")))) for x in input_data.splitlines()]
...
def task1(day_input):
    ...
    for p1, p2 in combinations(day_input, 2):
        area = np.prod(abs(p1 - p2) + 1)
        if area > max_area:
            max_area = area
            max_points = [p1, p2]

    print(f"{max_points} | dist={max_area}")
...
# 2165426818 X
# 102205235 X
# 2338426344 X too low
# 4781090232 X too low
# 4782268188


@timer(return_time=True)
def task2(day_input):
    polygon = Polygon(day_input)
    hist = []

    for p1, p2 in combinations(day_input, 2):
        area = np.prod(abs(p1 - p2) + 1)
        hist.append([p1, p2, area])

    hist = sorted(hist, key=lambda x: x[2], reverse=True)

    for p1, p2, area in hist:
